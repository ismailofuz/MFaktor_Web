...
public class SeatController {
    ...
    //TODO joy sotish kerak //visitor yoki userId orqali joy sotish kerak
    @PostMapping("/sell")
    public ResponseEntity<ApiResponse> sold(@RequestParam Long userId, @RequestParam Integer eventId, @RequestParam Integer seatId, @RequestBody EventSeatResp eventSeatResp) {
        ApiResponse apiResponse = seatService.sell(userId, eventId, seatId, eventSeatResp);
        return ResponseEntity.status(apiResponse.isSuccess() ? 201 : 409).body(apiResponse);
    }


    @GetMapping("/seatsByEvent")
    public ResponseEntity<ApiResponse> getInfo(@RequestParam Integer eventId) {
        ApiResponse response = seatService.getInfo(eventId);
        return ResponseEntity.status(203).body(response);
    }


    //TODO //aynan qaysidir eventga tegishli seatlar infosi
    //TODO //
